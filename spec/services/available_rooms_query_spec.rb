describe AvailableRoomsQuery do
  ...
  context 'a room booked' do
    let!(:booking) { create(:booking, room: room_1, checkin_time: 1.day.ago, checkout_time: 3.days.from_now)}
    it 'returns available rooms' do
      expect(service.execute(Time.current, 1.day.from_now)).to eq([room_2])
    end
  end
end
